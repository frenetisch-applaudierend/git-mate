...
pub fn run(args: InitArgs) -> Result<(), String> {
    let name =
        crate::git::config::read_string("mate.shorthand").unwrap_or_else(|| "gm".to_string());
...
fn build_shortcuts() -> String {
    let entries = [
        ("checkout", "_git_mate_checkout"),
        ("new", "_git_mate_new"),
        ("finish", "_git_mate_finish"),
        ("sync", "_git_mate_sync"),
    ];
    let arms: String = entries
        .iter()
        .filter_map(|(cmd, fn_name)| {
            crate::git::config::read_string(&format!("mate.{cmd}.shorthand"))
                .map(|sh| format!("        {sh}) {fn_name} \"${{@:2}}\" ;;\n"))
        })
        .collect();
...
fn shell_init(name: &str, shell: &Shell) -> String {
    ...
    INIT_TEMPLATE
        ...
        .replace("{NAME}", name)
        .replace("{SHORTCUTS}", &build_shortcuts())
}
...
function _git_mate_sync()     { _git_mate_run sync     "$@"; }
